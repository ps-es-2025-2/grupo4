...
' Define os participantes conforme o Diagrama de Classes e Arquitetura Lógica
...
### Descrição
Diagrama de Sequência para UC02 baseado no Fluxo Básico, Alternativo A1 e Exceção E1. Demonstra a colaboração entre diferentes serviços (Agendamento e Cadastro).
...
alt "E1: Conflito de Agenda" [disponivel == false]
  ...
  ' Passo 4: O usuário pesquisa e seleciona o Paciente... usando o CadastroService
...
alt "E1: Conflito" [agendamentos.size > 0]
  AgService --> Actor: return "Erro: Já existem agendamentos ativos no período."
...
' Passos 1, 2, 4: Ator acessa função, seleciona item e informa quantidade
...
alt "E1: Item Vencido" [itemEstoque.validade < hoje]
  ...
  Service --> Actor: return "Erro: Estoque insuficiente. Disponível: " + itemEstoque.quantidadeTotal
else "Fluxo Básico"
...
alt "E2: NF Duplicada" [nfJaExiste == true]
  ...
  loop para cada item em listaItens
    ...
    Service -> DAO: registrarEntradaItem(item.id, item.quantidade, item.validade, item.lote)
    activate DAO
    DAO --> Service: return
    deactivate DAO
  end
...
![UC07 - Gerar Alerta de Estoque Crítico](images/image2.png)
...
alt "E1: Paciente Não Encontrado" [paciente == null]
  ...
  AgService -> AgDAO: agendamentos := buscarAgendamentosPorPaciente(paciente.id)
  activate AgDAO
...
![UC08 - Consultar Histórico do Paciente](images/image3.png)
...
alt "E2: Agendamento Já Realizado" [ag.status == "Realizado"]
  AgService --> Actor: return "Erro: Não é possível cancelar um agendamento já realizado."
else "Fluxo Básico"
  ...
  AgService -> AgDAO: atualizarStatus(agendamentoId, "Cancelado", motivo)
  activate AgDAO
  AgDAO --> AgService: return
  deactivate AgDAO
