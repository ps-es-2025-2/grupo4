...
public class MedicamentoRepository {
  ...
  private final String prefix = "medicamento:";

  public Medicamento save(Medicamento m) {
    if (m.getIdItem() == null) {
      m.setIdItem(System.currentTimeMillis());
    }

    try {
      String key = prefix + m.getIdItem();
      String value = mapper.writeValueAsString(m);
      restTemplate.postForObject(baseUrl + "/set?key=" + key, value, String.class);
      return m;
    } catch (Exception e) {
      throw new RuntimeException("Erro ao salvar medicamento", e);
    }
  }
  ...
  public Medicamento findById(Long id) {
    try {
      ...
      return mapper.readValue(json, Medicamento.class);
    } catch (Exception e) {
      return null;
    }
  }
  ...
  public List<Medicamento> findAll() {
    try {
      ...
      return mapper.readValue(scanJson, new TypeReference<List<Medicamento>>() {
      });
    } catch (Exception e) {
      return new ArrayList<>();
    }
